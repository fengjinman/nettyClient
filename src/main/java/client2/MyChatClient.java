...
public class MyChatClient {
    public static void main(String[] args) throws Exception{
        ...
        try{
            ...
            ChannelFuture cf2 = bootstrap.connect("192.168.0.21", 8764).sync();
//
//            //写入
//            cf2.channel().writeAndFlush(Unpooled.copiedBuffer("因为爱情不是买卖".getBytes()));
//
//            cf2.channel().closeFuture().sync();

//            Channel channel = bootstrap.connect("192.168.0.21",8899).sync().channel();
//            Channel channel = bootstrap.connect("127.0.0.1",8764).sync().channel();
...
//                channel.writeAndFlush();
                cf2.channel().writeAndFlush(Unpooled.copiedBuffer((bufferedReader.readLine() +"\r\n").getBytes()));
            }
